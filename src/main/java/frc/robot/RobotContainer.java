...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    // trigger
    m_driverController.button(1).toggleOnTrue(new InstantCommand(() -> {
      grabberSubsystem.intake();
    }));
    m_driverController.button(1).toggleOnFalse(new InstantCommand(() -> {
      grabberSubsystem.stop();
    }));
    // sideButton
    m_driverController.button(2).toggleOnTrue(new InstantCommand(() -> {
      grabberSubsystem.outtake();
    }));
    m_driverController.button(2).toggleOnFalse(new InstantCommand(() -> {
      grabberSubsystem.stop();
    }));
    // fast intake
    m_driverController.button(5).toggleOnTrue(new InstantCommand(() -> {
      grabberSubsystem.fastOuttake();
    }));
    m_driverController.button(5).toggleOnFalse(new InstantCommand(() -> {
      grabberSubsystem.stop();
    }));
    // 7 = intake
